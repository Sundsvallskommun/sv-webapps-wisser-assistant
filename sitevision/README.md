# Wisser assistant - Sitevision webapp

## Developing

Create `.dev_properties.json` with your environment settings.
Create `appDataDefaults.json`.

When running `dev` and `build` scripts, the latest build of the assistant app will be copied to the repo.
Remeber to always build `../app` before running the Sitevision webapp.
